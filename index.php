<?php
// index.php - Page d'accueil = Pointage direct avec bouton Admin
session_start();
...
<head>
    ...
    <title>GaragePoint - Pointage</title>
    ...
    <style>
        ...
        .pin-status.success { color: var(--secondary); animation: fadeInUp 0.3s ease; }
        .pin-status.error { color: var(--accent); animation: fadeInUp 0.3s ease; }
        .pin-status .spinner {
            display: inline-block;
            width: 16px;
            height: 16px;
        ...
        .numpad button:active::after { width: 200px; height: 200px; }
        .numpad button:hover {
        ...
        .actions button:active::after { width: 300px; height: 300px; }
        ...
        .actions button:disabled::after { display: none; }
        ...
        .user-info .name { font-size: 20px; font-weight: 700; color: white; }
        .user-info .poste { color: var(--text-secondary); font-size: 14px; margin-top: 2px; }
        .user-info .code { color: var(--primary); font-family: monospace; font-size: 13px; margin-top: 4px; letter-spacing: 1px; }
        ...
        @media (max-width: 480px) {
            ...
            .scan-header .title-section h1 { font-size: 16px; }
            .scan-header .logo-wrapper img { height: 28px; width: 28px; }
            .scan-header .admin-btn { padding: 6px 12px; font-size: 11px; }
            .scan-header .admin-btn i { font-size: 12px; }
            .pin-digit { width: 48px; height: 60px; font-size: 26px; }
            .pin-digits { gap: 10px; }
            .numpad button { padding: 14px; font-size: 22px; }
            .actions button { font-size: 13px; padding: 14px; }
            .success-overlay .s-name { font-size: 28px; }
            .success-overlay .check { font-size: 72px; }
        }
    </style>
</head>
<body>
    ...
    <script>
        ...
        function pointer(type) {
            ...
            fetch('api/pointage.php', {
                ...
                body: JSON.stringify({ employe_id: currentUser.id, type: type })
            })
            .then(r => r.json())
            .then(result => {
                if (result.success) {
                    const heure = new Date().toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' });
                    showSuccess(currentUser.prenom + ' ' + currentUser.nom, type, heure, currentUser.poste, currentUser.pin_code);
                } else {
                    status.innerHTML = '❌ Erreur';
                    status.className = 'pin-status error';
                    playSound('error');
                    setTimeout(() => { clearPin(); isProcessing = false; }, 1500);
                }
            })
            .catch(() => {
                status.innerHTML = '❌ Erreur de connexion';
